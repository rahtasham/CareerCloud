	...
	public class ApplicantProfileLogic : BaseLogic<ApplicantProfilePoco>
	{
		...
		protected override void Verify(ApplicantProfilePoco[] pocos)
		{
			...
			foreach (ApplicantProfilePoco item in pocos)
			{
				...
				{
					exceptions.Add(new ValidationException(111, $"Current Salary for {item.Id} cannot be negative"));
				}
				if (item.CurrentRate <0)
				{
					exceptions.Add(new ValidationException(112, $"Current Rate for {item.Id} cannot be negative"));
				}
			}
